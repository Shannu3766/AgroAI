...
# --- NEW: Centralized list of keys required by format_prompt ---
...
def format_prompt(params):
    """
    Generates the full prompt text from a dictionary of parameters.
    ...
    return (
        ...
        f"Predict the suitable Crop Type and Fertilizer Name"
    )
...
def run_retraining_job():
    ...
    try:
        ...
        # The feedback data contains 'prompt', 'chosen', and 'rejected' fields.
        # The 'prompt' is the full text generated by `format_prompt` during the
        # original prediction, ensuring the model is retrained on the correct context.
...
def predict():
    ...
    # MODIFIED: Use the centralized list of keys for validation
    ...
    try:
        with model_lock:
            # The prompt is now created using the helper function and validated keys
            ...
            return jsonify({
                ...
                # ADDED: This field is crucial for the feedback loop. The client application
                # must send this exact string back to /feedback as the 'prompt' field.
