...
def load_model():
    ...
    # First check if we're already loading
    with model_lock:
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            logging.info("CUDA not available. Model is running on CPU.")
...
if __name__ == '__main__':
    ...
    if 'initial_load_thread' in globals() and initial_load_thread.is_alive():
        initial_load_thread.join()
    
    # For development, use threaded mode to avoid forking issues with CUDA
